export default {
	...
	getApplications: async () => {
		...
		const status = sel_area.selectedOptionValue;
	...
	getMonthlyInstallment: () => {
		const interestRate = tbl_coplaboradores.selectedRow.Interest;
		const totalPayments = tbl_coplaboradores.selectedRow.Term * 12;
		const requestedAmount = parseInt(tbl_coplaboradores.selectedRow.AmountOffered.replace(/[^\d.-]/g, ''));
		...
		return tbl_coplaboradores.selectedRow.AmountOffered ? parseInt(monthylyPayback.toFixed(2)).toLocaleString('en-US', { style: 'currency', currency: 'USD' }) : ''
	},

	returnCreditRiskColor: () => {
		if (tbl_coplaboradores.selectedRow.CreditRisk === 'Low') {
			return 'RGB(0, 128, 0)'
		};
		if (tbl_coplaboradores.selectedRow.CreditRisk === 'Medium') {
			return 'RGB(255, 165, 0)';
		};
		if (tbl_coplaboradores.selectedRow.CreditRisk === 'High') {
			return 'RGB(255, 0, 0)'
		}
		return 'RGB(255, 165, 0)'
	},
	...
	disbursePayment: async () => {
		if (!tbl_coplaboradores.selectedRow.Bank || !tbl_coplaboradores.selectedRow.BankNo) {
			return showAlert('Add bank details to continue', 'warning');
		}

		const amountOffered = parseInt(tbl_coplaboradores.selectedRow.AmountOffered.replace(/[^\d.-]/g, ''))

		const noOfMonths = tbl_coplaboradores.selectedRow.Term * 12 || 12;
		const interest = amountOffered * (tbl_coplaboradores.selectedRow.Interest / 100);
		const totalReturned = amountOffered + interest;

		const monthlyFee = totalReturned / noOfMonths;
		let startDate = new Date(tbl_coplaboradores.selectedRow.Created);
